...
class InfoDatabase : InfoDatabaseRepository {
    ...
    override suspend fun getAcceptedInfos(): List<Info> = makeTransaction {
        selectInfos()
                .where { InfoTable.accepted.eq(true) }
                .execute()
                .map(::toInfo)
                .toList()
    }
    ...
    private fun selectInfos() = InfoTable.select(InfoTable.id, InfoTable.title, InfoTable.description, InfoTable.sources, InfoTable.url, InfoTable.author, InfoTable.authorUrl, InfoTable.occurrence)
